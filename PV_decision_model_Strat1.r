...
## Prepare data
PrepareData <- {  
  format(read.csv("data/PV_Input_variables_strat1.csv")[,1:5], scientific=FALSE)
  ...
  make_variables(estimate_read_csv("data/PV_Input_variables_strat1.csv"))
}




benefit.sim.PV.strat1 <- function(){
  ...
  VER_Benefits <- data.frame(Year = integer(), 
                             ...
                             VERforSale = numeric(),
                             VER_Income = numeric(),
                             TotalVERBenefits_Strat3 = numeric(),
                             stringsAsFactors = F)
  ...
  CompanyShares <- data.frame(Year = integer(), 
                              TotalCompSharesBenefits_strat1 = numeric())
                              
  {
    vv_company_shares_additional_strat1 <- vv(company_shares_additional_strat1, var_CV, n=n_years)
     vv_company_shares_additional_df <- data.frame(Year$Year, 
                                         vv_company_shares_additional_strat1)
  }
  ...
  for (y in 1:n_years) {
    ...
    CompanyShares[y, "TotalCompSharesBenefits_strat1"] <- 
      vv_company_shares_additional_df[y, "vv_company_shares_additional_strat1"]*
      company_shares_price
    CompanyShares[1:2, "TotalCompSharesBenefits_strat1"] <- 0
  } 

  #######Benefits from additional product sales#######
  
  ProductSales <- data.frame(Year = integer(),
                             AddProductSalesStrat1 = numeric())
  {
    # Marketing strategy 1
    vv_productsales_additional_strat1 <- vv(productsales_additional_strat1, var_CV, n=n_years)
     vv_productsales_df <- data.frame(Year$Year, 
                                     vv_productsales_additional_strat1)
  }
  ...
  for (y in 1:n_years){
    ...
    # Marketing strategy 1
    ProductSales[y, "AddProductSalesStrat1"] <-
      vv_productsales_df[y, "vv_productsales_additional_strat1"]*NewArea[y, "CertfiedArea"]
     }
  ...
  for (y in 1:n_years) {
    ...
    if (MarketingCosts[y, "Year"]==1){
      MarketingCosts[y, "marketing_cost_strat1"] <- marketing_strat1}
    else {MarketingCosts[y, "marketing_cost_strat1"] <- 0}
    }
  ...
  # Strategy 1 
  Cashflow_PV_Strat1 <-  (ProductSales$AddProductSalesStrat1 + 
                            CompanyShares$TotalCompSharesBenefits_strat1)-
                          (SalaryCosts$TotalSalaryCosts +
                          GsFees$TotalGsFeesEUR + 
                          MarketingCosts$marketing_cost_strat1)
  
  NPV_PV_Strat1 <- discount(Cashflow_PV_Strat1, discount_rate = discount_rate,
                                    calculate_NPV = TRUE)
 
   
  return(list(NPV_PV_Strat1 = NPV_PV_Strat1,
             Cashflow_NPV_PV_Strat1_ = Cashflow_PV_Strat1))
  
  
} # End of benefit.sim.PV.strat1 function


decisionSupport(inputFilePath = "data/PV_Input_variables_strat1.csv", #input file with estimates
                outputPath = file.path(getwd(),"MCResults_PV_Strat1"), #output folder
                write_table = TRUE,
                welfareFunction = benefit.sim.PV.strat1,
...
mc_PV_1<-read.csv("MCResults_PV_Strat1/mcSimulationResults.csv")
legend_table_1<-read.csv("data/PV_legend_strat1.csv")
mc_EVPI_PV_1<-mc_PV_1[,-grep("Cashflow",colnames(mc_PV_1))]
dir.create("Figures_PV_Strat1")
empirical_EVPI(mc_EVPI_PV_1,"NPV_PV_Strat1",write_table=TRUE,fileformat="png",outfolder="Figures_PV",
               p_spearman=0.05, legend_table=read.csv("data/PV_legend_strat1.csv"),#legend_table,
               output_legend_table=read.csv("data/PV_legend_strat1.csv"))#legend_table)



for (variable_name in c("NPV_PV_Strat1"))
  compound_figure(variable_name=variable_name,
                  MC_table=mc_PV_1,
                  PLS_table=read.csv(paste("MCResults_PV_Strat1/",variable_name,"_pls_results.csv",sep="")),
                  ...
                  legend_table=read.csv("data/PV_legend_strat1.csv"))
