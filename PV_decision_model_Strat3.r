...
PrepareData <- {  
  format(read.csv("data/PV_Input_variables_strat3.csv")[,1:5], 
         scientific=FALSE)
  ...
  make_variables(estimate_read_csv("data/PV_Input_variables_strat3.csv"))
}




benefit.sim.PV.strat3 <- function(){
  ...
  CompanyShares <- data.frame(Year = integer(), 
                              TotalCompSharesBenefits_strat3 = numeric())
  
  {
    vv_company_shares_additional_strat3 <- vv(company_shares_additional_strat3, var_CV, n=n_years)
    vv_company_shares_additional_df <- data.frame(Year$Year,
                                                  vv_company_shares_additional_strat3)
  }
  ...
  for (y in 1:n_years) {
    ...
    CompanyShares[y, "TotalCompSharesBenefits_strat3"] <- 
      vv_company_shares_additional_df[y, "vv_company_shares_additional_strat3"]*
      company_shares_price
    CompanyShares[1:2, "TotalCompSharesBenefits_strat3"] <- 0
  } 
  
  #######Benefits from additional product sales#######
  
  ProductSales <- data.frame(Year = integer(),
                             AddProductSalesStrat3 = numeric())
  {
    # Marketing strategy 2
    vv_productsales_additional_strat3 <- vv(productsales_additional_strat3, var_CV, n=n_years)
    vv_productsales_df <- data.frame(Year$Year, 
                                     vv_productsales_additional_strat3)
  }
  ...
  for (y in 1:n_years){
    ...
    # Marketing strategy 2
    ProductSales[y, "AddProductSalesStrat3"] <-
      vv_productsales_df[y, "vv_productsales_additional_strat3"]*NewArea[y, "CertfiedArea"]
  }
  ...
  for (y in 1:n_years) {
    ...
    if (MarketingCosts[y, "Year"]==1){
      MarketingCosts[y, "marketing_cost_strat3"] <- marketing_strat3}
    else {MarketingCosts[y, "marketing_cost_strat3"] <- 0}
  }
  ...
  # Strategy 2
  Cashflow_PV_Strat3 <-  (CompanyShares$TotalCompSharesBenefits_strat3+
                            ProductSales$AddProductSalesStrat3)-
                            (SalaryCosts$TotalSalaryCosts +
                            GsFees$TotalGsFeesEUR + 
                            MarketingCosts$marketing_cost_strat3)
  
  NPV_PV_Strat3 <- discount(Cashflow_PV_Strat3, discount_rate = discount_rate,
                            calculate_NPV = TRUE)
  
  
  return(list(NPV_PV_Strat3 = NPV_PV_Strat3,
              Cashflow_NPV_PV_Strat3_ = Cashflow_PV_Strat3))
  
  
} # End of benefit.sim.PV.strat3 function


decisionSupport(inputFilePath = "data/PV_Input_variables_strat3.csv", #input file with estimates
                outputPath = file.path(getwd(),"MCResults_PV_Strat3"), #output folder
                write_table = TRUE,
                welfareFunction = benefit.sim.PV.strat3,
...
mc_PV_3<-read.csv("MCResults_PV_Strat3/mcSimulationResults.csv")
legend_table_3<-read.csv("data/PV_legend_strat3.csv")
mc_EVPI_PV_3<-mc_PV_3[,-grep("Cashflow",colnames(mc_PV_3))]
#dir.create("Figures_PV")
empirical_EVPI(mc_EVPI_PV_3,"NPV_PV_Strat3",write_table=TRUE,fileformat="png",outfolder="Figures_PV",
               p_spearman=0.05, legend_table=read.csv("data/PV_legend_strat3.csv"),#legend_table,
               output_legend_table=read.csv("data/PV_legend_strat3.csv"))#legend_table)


for (variable_name in c("NPV_PV_Strat3"))
  compound_figure(variable_name=variable_name,
                  MC_table=mc_PV_3,
                  PLS_table=read.csv(paste("MCResults_PV_Strat3/",variable_name,"_pls_results.csv",sep="")),
                  ...
                  legend_table=read.csv("data/PV_legend_strat3.csv"))
